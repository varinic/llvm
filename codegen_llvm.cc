...
//to get platform information in function "CodeGenLLVM::InitTarget"
//and deliver it to function 
#define debug
#ifdef debug
std::string triple;
#endif
...
void CodeGenLLVM::InitTarget(llvm::TargetMachine* tm) {
  ...
  //to get platform triple information.
  #ifdef debug
  triple="unknown";
  std::cout<<" ---> from CodeGenLLVM::InitTarget"<<std::endl;
  std::cout<<" native_vector_bits_ : "<<native_vector_bits_<<std::endl;
  const auto& arch = tm->getTargetTriple().getArch();
  if (arch == llvm::Triple::arm || arch == llvm::Triple::aarch64) {
    triple="arm";
    std::cout<<" arch == arm|aarch64 and triple = 'arm'"<<std::endl;
  }
  #endif
...
llvm::Value* CodeGenLLVM::CreateIntrinsic(const Call* op) {
  if (op->is_intrinsic("llvm_intrin")) {
    ...
    if(triple=="arm" && sig_type.size()==0){
      std::cout<<" **************************************************************************"<<std::endl;
      std::cout<<" * This is arm plateform , speical intrinsic rule should be implemented ! *"<<std::endl;
      std::cout<<" * As for @llvm.arm.neon.vmulls.v4i32 etc. you shall set const zero       *"<<std::endl;
      std::cout<<" * for num_signature, which means the number of arguments !               *"<<std::endl;
      std::cout<<" * We will add ReturnType into intrisic descripton automatically !        *"<<std::endl;
      std::cout<<" **************************************************************************"<<std::endl;
      sig_type.insert(sig_type.begin(), return_type);
    }
    else{
